...
//    * Ruftaster  -> serielle Konsole: '0'=EG, '1'=OG1, '2'=OG2, 'q'=Fehler quitt.
//    * Magnetkontakte (Endschalter) -> ZEITBASIERT: nach FAHRZEIT_PRO_ETAGE_MS je
//      Etagen-Abstand "schliesst" der Ziel-Endschalter; der Aufzug stoppt dann.
//  Spaeter werden diese zwei Simulationsbloecke durch DigitalInput ersetzt.
// =============================================================================
#include <Arduino.h>
#include "Config.h"
#include "Aufzug.h"
...
void setup() {
  ...
  Serial.println("Befehle: 0=EG  1=OG1  2=OG2  q=Fehler quittieren");
}

void loop() {
  // 1) Ruftaster simulieren (serielle Konsole)
  if (Serial.available()) {
  ...
  // 2) Magnetkontakte simulieren: Ziel-Reed schliesst nach Fahrzeit
  bool reed[3] = { false, false, false };
  if (gFahrtAktiv && (int32_t)(millis() - gAnkunftMs) >= 0)
    reed[(int)gZiel] = true;

  // 3) Aufzug-Steuerung: taktet den realen Motor, prueft Reeds + Timeout
  aufzug.update(reed[0], reed[1], reed[2]);
  if (aufzug.zustand() == Aufzug::Zustand::STEHT) gFahrtAktiv = false;
